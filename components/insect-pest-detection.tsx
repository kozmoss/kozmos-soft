...
export default function PestImageUpload() {
  ...
  const t = useTranslations("PlantDisease&PestDetection.PestDetection");
  ...
  const uploadFile = async (file: File) => {
    ...
    try {
      const response = await fetch("/api/insect/files/upload", {
        method: "POST",
        body: formData,
      });
      ...
      if (response.ok) {
        ...
        console.error("Hata yanıtı:", data);
        toast.error(data.error || "Yükleme başarısız oldu");
        return null;
      }
    } catch (error) {
  ...
  const getPredictions = () => {
    if (!results?.prediction || !Array.isArray(results.prediction) || results.prediction.length === 0) {
      return [];
    }
